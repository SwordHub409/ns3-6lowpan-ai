...
class DQN(object):
    ...
    def choose_action(self, x):
        ...
        if np.random.uniform() > 0.99 ** self.memory_counter:    # choose best
            ...
            action = torch.argmax(action, 0).cpu().numpy()
        else:    # explore
    ...
    def learn(self, ):
        ...
        s = torch.Tensor(sample[:, :self.observer_shape]).to(device)
        a = torch.LongTensor(
            sample[:, self.observer_shape:self.observer_shape+1]).to(device)
        r = torch.Tensor(
            sample[:, self.observer_shape+1:self.observer_shape+2]).to(device)
        s_ = torch.Tensor(sample[:, self.observer_shape+2:]).to(device)
